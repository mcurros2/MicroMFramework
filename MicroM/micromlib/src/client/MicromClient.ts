...
export class MicroMClient {
    ...
    async #startOidcLogin(options: {
        ...
        try {
            ...
            // State, Nonce, Code Challeng and Code Challenge Method are generated by the server
            ...
            if (extraParams) {
                for (const [k, v] of Object.entries(extraParams)) {
                    if (v != null) {
                        // Avoid overriding security-related parameters that are now server-managed.
                        if (
                            k === 'state' ||
                            k === 'nonce' ||
                            k === 'code_challenge' ||
