import { DataResult, DBStatusResult, FileImportMapping, ImpDataResult, MicroMClient, MicroMRequestOptions, ValuesObject } from "../client";
...
export class EntityAPI {
     ...
     * Import data
     */
    async importData(abort_signal: AbortSignal | null = null, import_procname: string | null = null, parentKeys: ValuesObject | null = null, fileprocess_id: string, fileImportMapping?: FileImportMapping, initialRow?: number): Promise<ImpDataResult> {
        const values = { c_fileprocess_id: fileprocess_id };
        const result = await this.client.import(this.#name, parentKeys, values, import_procname, abort_signal, fileImportMapping, initialRow);
        return result;
    }
