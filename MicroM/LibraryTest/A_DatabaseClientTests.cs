...
public class A_DatabaseClientTests
{
    ...
    public async Task ExecuteSQLChannel_ReturnsCorrectNumberOfResultsAndRecords()
    {
        ...
        var producer = client.ExecuteSQLChannel("select top 10 * from sysobjects\nselect top 10 * from sysobjects", result_set, 5, cts.Token);
    ...
    public async Task ExecuteSPChannel_ReturnsCorrectNumberOfResultsAndRecords()
    {
        ...
        var producer = client.ExecuteSPChannel("sp_help", parms.Columns, result_set, 5, cts.Token);
    ...
    [TestMethod]
    public async Task ExportToExcelSylvan_TestExport()
    {
        ...
        try
        {
            var fileResult = await ExportSylvanFromChannelAsync($"test_export", resultChannel, producer, cts.Token);
