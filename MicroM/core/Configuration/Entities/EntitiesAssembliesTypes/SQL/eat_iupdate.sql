create or alter proc [dbo].eat_iupdate
        @assembly_id Char(20)
...
begin try
    ...
    select  @cu=dt_lu
    from    [dbo].[entities_assemblies_types] with (rowlock, holdlock, updlock)
    ...
    begin
        ...
        declare @id bigint
		exec [dbo].num_iGetNewNumber 'eat', @nextnumber = @id out
		select @assemblytype_id = right('0000000000'+rtrim(@id),10)

        insert  [dbo].[entities_assemblies_types]
        values
            (
            @assembly_id
			, @assemblytype_id
