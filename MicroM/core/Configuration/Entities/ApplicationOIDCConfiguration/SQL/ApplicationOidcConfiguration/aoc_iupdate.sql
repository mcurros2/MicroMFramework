create or alter proc [dbo].aoc_iupdate
...
begin try
    ...
    select  @cu=dt_lu
    from    [dbo].[application_oidc_configuration] with (rowlock, holdlock, updlock)
    where   c_application_id = @application_id

    if @cu is null
    begin

        insert  [dbo].[application_oidc_configuration]
        values
            (
            @application_id
            , @certificate_id
    ...
    update  [dbo].[application_oidc_configuration]
