create or alter proc [dbo].aoi_update
...
BEGIN
    ...
    from    openjson(@url_authorized_redirects) WITH (vc_url_authorized_redirect varchar(2048) '$') a
            left join [dbo].application_oidc_clients_authorized_urls b
...
begin try
    ...
    select  @cu=a.dt_lu
            ...
            , @apikey_lu = b.dt_lu
    from    [dbo].[application_oidc_clients] a with (rowlock, holdlock, updlock)
    ...
    begin
        ...
        declare @id bigint
        exec [dbo].num_iGetNewNumber 'aoi', @nextnumber = @id out
        select @client_app_id = right('0000000000'+rtrim(@id),10)

        exec [dbo].mak_iupdate 
        ...
        insert  [dbo].[application_oidc_clients]
        values
            (
            @application_id
            , @client_app_id
        ...
        if @url_authorized_redirects is not null
        begin
            insert  [dbo].application_oidc_clients_authorized_urls
    ...
    update  [dbo].[application_oidc_clients]
    ...
    where   c_client_app_url_id not in 
            (   
                ...
                from    [dbo].application_oidc_clients_authorized_urls x
