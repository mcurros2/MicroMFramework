...
internal static class CategoriesAndStatusExtensions
{
    ...
    internal static List<string> CreateCategoryOrStatusTable(this EntityBase entity, bool is_status)
    {
        ...
        string table_name = is_status ? entity.AsEntityStatusTable() : entity.AsEntityCategoryTable();
        ...
        if (is_status)
        {
            ...
            ret.AppendFormat(CultureInfo.InvariantCulture, "CONSTRAINT FK{0}_{1} FOREIGN KEY ({2},{3}) REFERENCES {4},\n", entity.Def.Mneo, $"{entity.Def.Mneo}s", nameof(status.Def.c_status_id), nameof(status.Def.c_statusvalue_id), status.Def.FullTableName);

            fk_index = $"create index FKIDX{entity.Def.Mneo}_stv on {entity.AsEntityStatusTable()} ({nameof(StatusValuesDef.c_status_id)}, {nameof(StatusValuesDef.c_statusvalue_id)})";
        }
        else
        {
            ...
            ret.AppendFormat(CultureInfo.InvariantCulture, "CONSTRAINT FK{0}_{1} FOREIGN KEY ({2},{3}) REFERENCES {4},\n", entity.Def.Mneo, $"{entity.Def.Mneo}s", nameof(cat.Def.c_category_id), nameof(cat.Def.c_categoryvalue_id), cat.Def.FullTableName);

            fk_index = $"create index FKIDX{entity.Def.Mneo}_cav on {entity.AsEntityCategoryTable()} ({nameof(CategoriesValuesDef.c_category_id)}, {nameof(CategoriesValuesDef.c_categoryvalue_id)})";
        }
    ...
    internal static string AsCategoriesAndStatusJoin<T>(this T entity, string separator = $"\n{TAB}{TAB}", string parent_alias = "a", string alias = "b") where T : EntityBase
    {
        ...
        var cat_table = entity.AsEntityCategoryTable();
        var stat_table = entity.AsEntityStatusTable();
        ...
        foreach (var cav_col in Fakes)
        {
            if (cav_col.SQLMetadata.IsArray == false)
            {
                if (entity.Def.RelatedCategories.Contains(cav_col.RelatedCategoryID!))
                {
                    ...
                    sb.Append(CultureInfo.InvariantCulture, $"{separator}{(cav_col.SQLMetadata.Nullable ? "left " : "")}join {cat_table} {alias}{separator}on({key_mappings}{separator}and {alias}.{cat.Def.c_category_id.Name} = '{cav_col.RelatedCategoryID!.SQLEscape()}')");
                ...
                else
                {
                    if (entity.Def.RelatedStatus.Contains(cav_col.RelatedStatusID!))
                    {
                        ...
                        sb.Append(CultureInfo.InvariantCulture, $"{separator}{(cav_col.SQLMetadata.Nullable ? "left " : "")}join {stat_table} {alias}{separator}on({key_mappings}{separator}and {alias}.{stat.Def.c_status_id.Name} = '{cav_col.RelatedStatusID!.SQLEscape()}')");
