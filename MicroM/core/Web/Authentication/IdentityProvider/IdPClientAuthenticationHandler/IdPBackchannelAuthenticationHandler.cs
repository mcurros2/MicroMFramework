...
public class IdPBackchannelAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    // MMC: this cache is different from the certificate cache - this is for client JWKS fetched from URLClientJWKS and stores keysets, not certificates
    private static readonly ConcurrentDictionary<string, (JsonWebKeySet jwks, DateTime expires)> _jwksCache = new();
            ...
            try
            {
                    ...
                    clientAssertionType == WellknownIdentityConstants.ClientAssertionTypeJwtBearer)
                {
                    ...
                    if (signingKeys == null || signingKeys.Length == 0)
                    {
                        ...
                        try
                        {
                            var jwks = await GetJwksCachedAsync(clientCfg.URLClientJWKS, Context.RequestAborted);
    ...
    private async Task<JsonWebKeySet> GetJwksCachedAsync(string jwksUrl, CancellationToken ct)
    {
        if (_jwksCache.TryGetValue(jwksUrl, out var entry) && entry.expires > DateTime.UtcNow)
        {
            return entry.jwks;
        }

        using var http = _httpFactory.CreateClient(ConfigurationDefaults.HTTPClientOidcName);
        using var res = await http.GetAsync(jwksUrl, ct);
        res.EnsureSuccessStatusCode();
        var jwksJson = await res.Content.ReadAsStringAsync(ct);
        var jwks = new JsonWebKeySet(jwksJson);

        var expires = DateTime.UtcNow.Add(TimeSpan.FromSeconds(ConfigurationDefaults.JwksCacheDurationSeconds));
        _jwksCache[jwksUrl] = (jwks, expires);
        return jwks;
    }
