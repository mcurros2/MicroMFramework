...
namespace MicroM.Web.Authentication.SSO
{
    public interface IEtagCacheService
    {
        ...
        EtagContent GetOrAdd(string key, Func<string> valueFactory);

        ValueTask<EtagContent> GetOrAddAsync(string key, Func<CancellationToken, ValueTask<string>> valueFactory, bool serveStaleOnError, CancellationToken ct, int maxRetries = 2);
