create or alter proc [dbo].aos_deleteSessionsBySUB @sub varchar(255) as
...
begin try
    ...
    from	[dbo].application_oidc_active_sessions a 
            join [dbo].microm_users_devices d
            on d.c_user_id = a.c_user_id
    where	a.vc_oidc_sub = @sub

    delete [dbo].application_oidc_active_sessions where vc_oidc_sub = @sub
