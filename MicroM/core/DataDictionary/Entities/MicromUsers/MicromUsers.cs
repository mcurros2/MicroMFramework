...
public class MicromUsers : Entity<MicromUsersDef>
{
    ...
    public async static Task<LoginData?> GetUserData(ApplicationOption app, string? username, string? user_id, string device_id, IEntityClient ec, CancellationToken ct)
    {
        ...
        return await user.Data.ExecuteProcSingleRow<LoginData>(proc, ct, set_parms_from_columns: false, mode: AutoMapperMode.ByNameLaxNotThrow);
    }
    ...
    public async static Task<DBStatusResult> SetTotpSecret(ApplicationOption app, string username, string totpSecret, IEntityClient ec, CancellationToken ct)
    {
        MicromUsers user = new(ec, schema_name: app.SchemaConfiguration.DDSchema);

        var proc = user.Def.usr_setTotpSecret;
        proc[nameof(MicromUsersDef.vc_username)].ValueObject = username;
