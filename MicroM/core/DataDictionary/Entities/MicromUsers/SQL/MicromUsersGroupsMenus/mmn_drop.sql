create or alter proc [dbo].mmn_drop
...
begin try
    ...
    (
        ...
        from    [dbo].[microm_menus_items] mi
    ...
    where   c_user_group_id = @user_group_id
            and exists (select 1 from Hierarchy h
                        where h.c_menu_id = [dbo].[microm_users_groups_menus].c_menu_id
                              and h.c_menu_item_id = [dbo].[microm_users_groups_menus].c_menu_item_id)
