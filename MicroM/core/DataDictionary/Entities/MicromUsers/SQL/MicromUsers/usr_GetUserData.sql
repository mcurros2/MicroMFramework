create or alter proc [dbo].usr_getUserData @username varchar(255), @user_id char(20), @device_id varchar(255) as

declare @now datetime = getdate(), @groups varchar(max)


select  @groups = '[' + STRING_AGG('"'+replace(RTRIM(c_user_group_id), '"','\"')+'"', ',') + ']'
from    [dbo].microm_users_groups_members a
...
select	[user_id] = rtrim(a.c_user_id)
		...
		, totp_confirmed_utc = a.dt_totp_confirmed
...
from	[dbo].microm_users a
		left join [dbo].microm_users_devices b
		on(b.c_user_id=a.c_user_id and b.c_device_id=@device_id)
		left join 
		(	[dbo].microm_users_cat c
			join [dbo].categories_values d
